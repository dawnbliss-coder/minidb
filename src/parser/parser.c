...
static ParsedStatement* parse_select(Parser* parser) {
    ...
    TokenType tok_type = parser->current_token->type;
    if (tok_type == TOKEN_COUNT || tok_type == TOKEN_SUM || tok_type == TOKEN_AVG ||
        tok_type == TOKEN_MAX || tok_type == TOKEN_MIN) {
        switch (tok_type) {
            case TOKEN_COUNT: stmt->agg_type = AGG_COUNT; break;
            case TOKEN_SUM:   stmt->agg_type = AGG_SUM;   break;
            case TOKEN_AVG:   stmt->agg_type = AGG_AVG;   break;
            case TOKEN_MAX:   stmt->agg_type = AGG_MAX;   break;
            case TOKEN_MIN:   stmt->agg_type = AGG_MIN;   break;
            default: break;
        }
